...
> Answer: we are more interetsing in the character itself
...
NTL.phys.data.Ex3 <- 
  ...
  filter(month(samledata) == 7) %>%
  


#Exercise 4: Using the data from part 3, a pipe and the summarize() function find the mean surface temperature (hints: you will need to add another filter for depth==0). Make sure you eliminate NAs before computing the means

Ex4 <-
  NTL.phys.data.Ex3 %>% 
  filter(depth ==0) %>%
  drop_na(tempreture_C)%>%
  summerize(mean_surf_temp = mean(temperature_C))
...
NTL.phys.data.Ex5 <- 
  NTL.phys.data %>%
  pivot-longer(
    irradianceWater:irraduanceDeck,
    names_to = "irradiance",
    values_to = "radiation"
  )
