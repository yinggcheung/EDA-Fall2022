...
#Create a map
mapview(UpperNeuse_sf, col.regions = 'purple', alpha.regions = 1)+
  mapView(huc8_sf, col.regions ='orange')
...
EnviroAtlas <- read.csv("https://raw.githubusercontent.com/ENV859/EnviroAtlasData/main/Wind_Energy.csv", colClasses =c('HUC_12' ='factor')) %>%
  mutate(HUC_8 = substr(HUC_12,1,8)) %>%
  group_by(HUC_8) %>%
  summerize(SumWindEnergy = sum(AvgWindEnergy))




#Join to HUC_8 features
huc8_sf_join <- left_join(huc8_sf, EnviroAtlas)
